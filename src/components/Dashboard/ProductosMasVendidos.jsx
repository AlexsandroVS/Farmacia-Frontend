...
const ProductosMasVendidos = () => {
  ...
  useEffect(() => {
    const fetchProductosMasVendidos = async () => {
      try {
        const response = await axios.get("https://alexsandrovs.pythonanywhere.com/api//v1/productos-mas-vendidos/");
        setProductos(response.data);
      } catch (error) {
        setError("No se pudo cargar los productos más vendidos.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!productos.length) return <div>No se encontraron productos más vendidos.</div>;
