...
const ReportesGraficosClientes = () => {
  ...
  const obtenerGananciasPorMes = async (year, month) => {
    try {
      const response = await fetch(
        `https://alexsandrovs.pythonanywhere.com/api/v1/reporte-mensual-cliente/${year}/${month}/`
      );
      const data = await response.json();
      console.log("Datos recibidos:", data); 
      return parseFloat(data.total_facturado);
    } catch (error) {
  ...
  const dataGanancias = gananciasMensuales;

  const data = {
    labels: labels,
    datasets: [
      {
        label: `Ganancia Total en ${year}`,
        data: dataGanancias,
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <div className="flex flex-col flex-1 overflow-auto">
        ...
        <main className="p-6 bg-gray-100 min-h-screen pt-10">
          ...
          <Breadcrumb
            paths={[
              { name: "Reporte General Cliente", link: "/reportes/clientes/" },
              { name: "Ganancias", link: "/reportes/clientes/grafico/" },
            ]}
          />
...
export default ReportesGraficosClientes;
