...
const VerFacturaCliente = () => {
  ...
  useEffect(() => {
    const fetchFactura = async () => {
      ...
      if (!token) {
        console.error(
          "Token no encontrado. No se puede realizar la solicitud."
        );
        return;
      }
      ...
      try {
        const response = await axios.get(
          `https://alexsandrovs.pythonanywhere.com/api/v1/facturas-cliente/${id}/`,
          {
            headers: {
      ...
      <div className="flex flex-col flex-grow overflow-hidden">
        ...
        <section className="flex-grow p-8 lg:px-12 overflow-y-auto transition-transform duration-300 ease-in-out">
          <Breadcrumb
            ...
            paths={[
              {
                name: "Lista de Facturas Clientes",
                link: "/facturas/clientes",
              },
              { name: `Detalle de Factura Cliente Nro° ${factura.id}` },
            ]}
          />
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg border border-gray-300 transition-all duration-500 ease-in-out transform hover:scale-105">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="w-full md:w-1/3">
                <p className="text-xl font-semibold text-gray-900">
                  <strong>IGV (18%):</strong> {factura.igv.toFixed(2)}
                </p>
              </div>
            </div>
